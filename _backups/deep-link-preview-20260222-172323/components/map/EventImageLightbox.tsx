...
useEffect(() => {
  ...
  const handleShare = async () => {
    ...
    try {
      // Build deep link URL for the event
      const shareUrl = buildGathrShareUrl(updatedEvent);

      const shareResult = await Share.share({
        message: `Check out ${updatedEvent.title} at ${updatedEvent.venue} on ${formatEventDateTime(
          updatedEvent.startDate,
          updatedEvent.startTime
        )}.\n\n${updatedEvent.description}\n\nSee it on GathR: ${shareUrl}`,
        title: updatedEvent.title,
        url: shareUrl, // iOS only - shows as link preview
      });
