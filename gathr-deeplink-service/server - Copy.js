...
function buildOgImageUrl(type, id) {
  return `https://${CONFIG.domain}/og/${type}/${encodeURIComponent(String(id || ''))}.png?v=${encodeURIComponent(CONFIG.ogImageVersion)}`;
}
...
async function handleLandingPage(req, res, type) {
  ...
  const ogImageUrl = buildOgImageUrl(normalizedType, id);
