...
async function fetchRemoteImageDataUri(imageUrl) {
  ...
  const timeout = setTimeout(
    () => controller.abort(),
    Number(process.env.OG_IMAGE_FETCH_TIMEOUT_MS || 5000)
  );
...
<svg width="${OG_WIDTH}" height="${OG_HEIGHT}" viewBox="0 0 ${OG_WIDTH} ${OG_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <linearGradient id="panelBg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="rgba(9,22,40,0.90)" />
      <stop offset="55%" stop-color="rgba(10,28,50,0.86)" />
      <stop offset="100%" stop-color="rgba(11,33,62,0.84)" />
    </linearGradient>
    <linearGradient id="panelBorder" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="rgba(255,255,255,0.26)" />
      <stop offset="100%" stop-color="rgba(127,196,255,0.14)" />
    </linearGradient>
    <linearGradient id="chipBg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="rgba(127,196,255,0.18)" />
      <stop offset="100%" stop-color="rgba(74,144,226,0.20)" />
    </linearGradient>
  </defs>
  ...
  <rect x="58" y="58" width="1084" height="514" rx="26" ry="26" fill="url(#panelBg)" stroke="url(#panelBorder)" />

  <rect x="72" y="72" width="252" height="52" rx="18" ry="18" fill="url(#chipBg)" />
  ${logoSvg}
  ${textLogoSvg}
...
async function generateOgImageBuffer(options) {
  const { cacheKey, svg } = await buildOgImageSvg(options);
