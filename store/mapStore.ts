...
fetchEventDetails: async (eventIds: (string | number)[]) => {
  ...
  const coerceDetailToEvent = (detail: any): Event => {
    ...
    return {
      ...
      actionLinks: Array.isArray(detail?.actionLinks) ? detail.actionLinks : [],
    ...
    const mergeCollection = (collection: Event[]): Event[] => {
      const merged = collection.map((event) => {
        ...
        return {
          ...
          ticketsBuyUrl: detail.ticketsBuyUrl ?? event.ticketsBuyUrl,
          ticketLink: detail.ticketLink ?? event.ticketLink,
          actionLinks: detail.actionLinks ?? event.actionLinks,
