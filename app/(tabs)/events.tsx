...
setSelectedImageData({ imageUrl, event });
  ...
  const eventsWithAds = useMemo<ListItem[]>(() => {
    ...
    // Use filteredOutsideViewportEvents.length for accurate total count
    if (filteredOutsideViewportEvents.length > 0) {
      result.push({
        type: 'divider',
        data: {
          message: 'Events outside your current map view',
          count: filteredOutsideViewportEvents.length
        }
      });
    }
  ...
  }, [getAdListKey, isFocusedContentReady, sortedViewportEvents, sortedOutsideViewportEvents, filteredOutsideViewportEvents.length, nativeAds, outsideViewportLoadCount]);
  ...
  const interestMatchSet = useMemo(() => {
    return measureListTabStage('events', 'interest_match_set', {
      ...
      eventsWithAds.forEach(item => {
        if (item.type === 'event' && doesEventMatchAnyInterest(item.data, userInterestLowerSet)) {
  ...
  }, [eventsWithAds.length, analytics, handleLoadMoreOutsideViewport]);
        ...
        <TouchableOpacity
          ...
          >
            <MaterialIcons 
              name="access-time" 
              size={14} 
              color={filterCriteria.eventFilters.timeFilter === TimeFilterType.NOW ? '#FFFFFF' : '#D5DAE2'}
          ...
          >
            <MaterialIcons 
              name="today" 
              size={14} 
              color={filterCriteria.eventFilters.timeFilter === TimeFilterType.TODAY ? '#FFFFFF' : '#D5DAE2'}
              style={styles.timeFilterIcon}
            />
            <Text style={[
              styles.timeFilterText,
          ...
          >
            <MaterialIcons 
              name="wb-sunny" 
              size={14} 
              color={filterCriteria.eventFilters.timeFilter === TimeFilterType.TOMORROW ? '#FFFFFF' : '#D5DAE2'}
              style={styles.timeFilterIcon}
            />
            <Text style={[
              styles.timeFilterText,
          ...
          >
            <MaterialIcons 
              name="event" 
              size={14} 
              color={filterCriteria.eventFilters.timeFilter === TimeFilterType.UPCOMING ? '#FFFFFF' : '#D5DAE2'}
              style={styles.timeFilterIcon}
            />
            <Text style={[
              styles.timeFilterText,
     ...
     {/* Event list with ads */}
      <FlatList
          ...
          renderItem={({ item, index }) => {
            ...
            // item.type === 'event'
            // Use pre-computed firstEventIndex (O(1)) instead of slice+filter (O(n))
            ...
            const eventListItem = (
              <MemoizedEventListItem
                ...
                matchesUserInterests={interestMatchSet.has(eventId)}
                isSaved={savedEventSet.has(eventId)}
                isGuest={isGuest}
                analytics={analytics || {}}
