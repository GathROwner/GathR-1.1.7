...
import FriendCheckInBadge from '../../components/social/FriendCheckInBadge';
...
const ANDROID_CLUSTER_MARKER_IMAGES = {
  'gathr-friend-only': require('../../assets/map-markers/friend-only.png'),
  'gathr-friend-attached': require('../../assets/map-markers/friend-attached.png'),
    ...
    .map((cluster): AndroidClusterMarkerFeature => {
      ...
      return {
        ...
        properties: {
          ...
          friendCountLabel: hasFriendPresence ? String(cluster.friendPresence!.friendCount > 9 ? '9+' : cluster.friendPresence!.friendCount) : '',
          ...
          friendOnly: hasFriendPresence && cluster.eventCount === 0 && cluster.specialCount === 0,
          hasFriendPresence,
          hasFriendEvents,
...
const getVenueLensMediaOptions = (
  ...
  const collectOptions = (matchActiveCategory: boolean): VenueLensMedia[] =>
    cluster.venues.flatMap((venue, venueIndex): VenueLensMedia[] => {
      const candidateEvents = matchActiveCategory
        ? venue.events.filter((event) => event.category.trim().toLowerCase() === normalizedCategory)
        : venue.events;
      if (candidateEvents.length === 0) return [];

      const avatarEvent = candidateEvents.find((event) => isRemoteMarkerImageUrl(event.profileUrl));
      if (avatarEvent) {
        return [{
          imageUrl: avatarEvent.profileUrl.trim(),
          venueIndex,
      ...
      if (!allowHeroFallback) return [];

      const heroEvent = candidateEvents.find((event) =>
        isRemoteMarkerImageUrl(event.imageUrl) || isRemoteMarkerImageUrl(event.SharedPostThumbnail)
      );
      if (!heroEvent) return [];

      return [{
        imageUrl: isRemoteMarkerImageUrl(heroEvent.imageUrl)
          ? heroEvent.imageUrl.trim()
          : heroEvent.SharedPostThumbnail.trim(),
        venueIndex,
        venueName: venue.venue,
        source: 'hero',
      }];
  ...
  const categoryOptions = collectOptions(true);
  return categoryOptions.length > 0 ? categoryOptions : collectOptions(false);
};

interface VenueIndicatorRailProps {
  activeVenueOrdinal: number;
  size: number;
  venueCount: number;
}

const VenueIndicatorRail: React.FC<VenueIndicatorRailProps> = ({
  activeVenueOrdinal,
  size,
  venueCount,
}) => {
...
const TreeMarker: React.FC<TreeMarkerProps> = React.memo(({ cluster, isSelected, isProcessing = false, isReady = true, detailsEnabled = true, isActive = true, appearance = 'tree', detailMode, presentation = 'story', reduceMotionEnabled = false, tutorialTargetRef, tutorialGeometryRef, onTutorialGeometryReady, tutorialBindingRevision = 0 }) => {
  ...
  const venueLensMediaOptions = useMemo(
    () => shouldLoadVenueLens
      ...
      : [],
    [activeCategory, allowHeroFallback, cluster, shouldLoadVenueLens]
  );
  const venueLensMediaKey = venueLensMediaOptions
    .map((option) => `${option.venueIndex}:${option.imageUrl}`)
    .join('|');
  const [activeVenueMediaIndex, setActiveVenueMediaIndex] = useState(0);

  useEffect(() => {
    setActiveVenueMediaIndex(0);
    if (!isActive || reduceMotionEnabled || venueLensMediaOptions.length < 2) return undefined;

    const interval = setInterval(() => {
      setActiveVenueMediaIndex((index) => (index + 1) % venueLensMediaOptions.length);
    }, CLUSTER_STAGE_CYCLE_MS);

    return () => clearInterval(interval);
  }, [isActive, reduceMotionEnabled, venueLensMediaKey, venueLensMediaOptions.length]);
  ...
  const friendPresence = cluster.friendPresence;
  const friendEventCount = detailsEnabled ? (cluster.friendEventCount || 0) : 0;

  if (friendPresence && cluster.eventCount === 0 && cluster.specialCount === 0) {
    return (
      <View style={[styles.markerWrapper, { opacity: !isReady ? 0.4 : isProcessing ? 0.6 : 1 }]}><FriendCheckInBadge count={friendPresence.friendCount} standalone /></View>
    );
  }
  ...
  return (
    ...
    >
      ...
      {detailsEnabled && isBeacon && isMultiVenue && (
        <VenueIndicatorRail
          activeVenueOrdinal={activeVenueOrdinal}
          size={adjustedSize}
      ...
      {detailsEnabled && !isBeacon && (cluster.eventCount > 0 || cluster.specialCount > 0) && (
        <View
          style={[
            styles.markerLabel,
  ...
  return (
    ...
    prevProps.cluster.friendPresence === nextProps.cluster.friendPresence &&
    prevProps.cluster.friendEventCount === nextProps.cluster.friendEventCount &&
...
useEffect(() => {
  ...
  const tutorialClusterInteractionReadyGate = useMemo(
    () => createTutorialBooleanGate(true),
    [],
  );
  useEffect(
    () => () => tutorialClusterInteractionReadyGate.dispose(),
    [tutorialClusterInteractionReadyGate],
  );
  ...
  useLayoutEffect(() => {
    ...
    const guardUntil = Math.max(
      existingGlobalGuardUntil,
      Date.now() + SHARED_EVENT_RETURN_INTERACTION_GUARD_MS
    );
    (globalThis as any).__gathrSharedEventReturnGuardUntil = guardUntil;
    calloutOpenTouchGuardUntilRef.current = guardUntil;
    filterPanelTouchGuardUntilRef.current = guardUntil;
    sharedEventReturnGuardUntilRef.current = guardUntil;
    clusterProcessingRef.current = null;
    setProcessingClusterId(null);
    tutorialClusterInteractionReadyGate.publish(!ignoreProgrammaticCameraRef.current);
...
// ------------------------------------------------
  ...
  const setIgnoreProgrammaticTrace = useCallback((value: boolean, reason: string) => {
    ...
    tutorialClusterInteractionReadyGate.publish(
      !value && clusterProcessingRef.current === null,
    );
    setMapTraceSnapshot({
...
// Prefer preloaded events; derive clusters immediately. Fallback only if no events.
  ...
  useEffect(() => {
    ...
    if (closeCalloutTrigger > previousCloseCalloutTrigger.current) {
      ...
      if (sharedEventReturnGuardRemainingMs > 0) {
        console.log('[MapScreen] Ignoring close callout trigger during shared event return guard', {
  ...
  const handleMarkerPress = useCallback(async (cluster: Cluster): Promise<boolean> => {
    ...
    tutorialClusterInteractionReadyGate.publish(false);
...
lastOpenedClusterIdRef.current = cluster.id;
    ...
    try {
      ...
      return shouldOpenCalloutForMarker && sortedVenues.length > 0;
    } catch (error) {
  ...
  useEffect(() => registerTutorialAction('open-cluster', async (signal?: AbortSignal) => {
    const currentTarget = tutorialCurrentClusterRef.current;
    const anchorVenueKey = tutorialTargetAnchorVenueKeyRef.current;
    ...
    if (
      !currentTarget ||
      !anchorVenueKey ||
      String(currentTarget.id) !== capturedBinding.clusterId ||
      useTutorialUiStore.getState().currentStepId !== 'cluster-click'
    ) {
      return false;
    }
    ...
    if (
      ...
      setIgnoreProgrammaticTrace(false, 'tutorial_open_cluster_cleared_initial_lock');
    }
    const interactionReady = await tutorialClusterInteractionReadyGate.waitFor(true, {
      timeoutMs: TUTORIAL_CLUSTER_OPEN_READY_TIMEOUT_MS,
      signal,
    });
    if (
      !interactionReady ||
      signal?.aborted ||
      useTutorialUiStore.getState().currentStepId !== 'cluster-click' ||
      !isTutorialClusterBindingCurrent(capturedBinding, {
        clusterId: tutorialTargetClusterIdRef.current,
        revision: tutorialTargetBindingRevisionRef.current,
      })
    ) {
    ...
    const interactionStarted = await handleMarkerPress(currentTarget);
    return didTutorialClusterOpen(
      interactionStarted,
  ...
  const handleMapPress = () => {
    ...
    if (sharedEventReturnGuardRemainingMs > 0) {
      logCalloutProbe('[CalloutProbe] handleMapPress ignored by shared event return guard', {
        sharedEventReturnGuardRemainingMs,
      });
      return;
    }
      ...
      return (
        ...
        >
          ...
          <MapboxGL.SymbolLayer
            ...
            filter={['!=', ['get', 'friendOnly'], true] as any}
            style={{
          ...
          <MapboxGL.SymbolLayer
            ...
            filter={['all', ['==', ['get', 'hasVenueIconOutline'], true], ['!=', ['get', 'friendOnly'], true]] as any}
            style={{
              iconAllowOverlap: true,
              iconAnchor: 'center',
              iconIgnorePlacement: true,
          ...
          <MapboxGL.SymbolLayer
            ...
            filter={['all', ['==', ['get', 'hasFriendPresence'], true], ['==', ['get', 'friendOnly'], true]] as any}
            style={{ iconAllowOverlap: true, iconIgnorePlacement: true, iconImage: 'gathr-friend-only', iconSize: 0.5, iconTranslate: [0, 0], iconTranslateAnchor: 'viewport', symbolSortKey: ['get', 'markerSortKey'] as any }}
          />
          <MapboxGL.SymbolLayer
            id="android-cluster-layer-friend-only-count"
            filter={['all', ['==', ['get', 'hasFriendPresence'], true], ['==', ['get', 'friendOnly'], true]] as any}
            style={{ textAllowOverlap: true, textIgnorePlacement: true, textField: ['get', 'friendCountLabel'] as any, textColor: '#FFFFFF', textSize: 12, textTranslate: [8, -3], textTranslateAnchor: 'viewport', symbolSortKey: ['get', 'markerSortKey'] as any }}
          />
          <MapboxGL.CircleLayer
...
countText: {
  ...
  interestPillsContainer: {
    ...
    bottom: 172, // Reserve the compact Friends shortcut above recenter.
    justifyContent: 'center',
    alignItems: 'flex-end',
    zIndex: 11,
  },
